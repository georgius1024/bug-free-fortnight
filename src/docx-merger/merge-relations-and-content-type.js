...
export function mergeContentTypes(files, _contentTypes) {
...
export function mergeRelations(files, _rel) {
  files.forEach(function (zip) {
    // var zip = new JSZip(file);
...
export function generateContentTypes(zip, _contentTypes) {
  // body...
  var xmlString = zip.file("[Content_Types].xml").asText();
  var xml = new DOMParser().parseFromString(xmlString, "text/xml");
...
export function generateRelations(zip, _rel) {
  // body...
  var xmlString = zip.file("word/_rels/document.xml.rels").asText();
  var xml = new DOMParser().parseFromString(xmlString, "text/xml");
  var serializer = new XMLSerializer();
